module AresMUSH
  module Pf2emagic
    class PF2MagicRepertoireCmd
      include CommandHandler

      attr_accessor :character, :charclass, :charclasses, :spell_level

      def parse_args
        # Usage: repertoire [character=][class/level]
        ...
        if cmd.args
          ...
          # If no args, the enactor is asking to see their whole repertoire. All three are nil.
        end
      ...
      def handle
        ...
        if rep.empty?
          client.emit_failure t('pf2emagic.no_repertoire', :item => char.name)
          return
        end

        # If a charclass was specified, is there anything in the repertoire for that charclass?
        if self.charclass && !(rep.keys.include? self.charclass)
          client.emit_failure t('pf2emagic.no_repertoire', :item => self.charclass)
          return
        end

        # This will not be called in the template unless self.charclass is specified.
        cc = self.charclass ? self.charclass : 'not specified'
        book = self.charclass ? rep[self.charclass] : rep
        ...
        if self.spell_level
          ...
          unless levelbook
            client.emit_failure t('pf2emagic.repertoire_no_spells_at_level', :options => book.keys.join(", "))
            return
          end
        ...
        template = PF2SpellbookTemplate.new(char, cc, book, client, 'pf2emagic.repertoire_title')
